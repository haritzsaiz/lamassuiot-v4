...
func StringToLoggerLevelHookFunc() mapstructure.DecodeHookFuncType {
	return func(
		f reflect.Type,
		t reflect.Type,
		data interface{},
		...
		if t != reflect.TypeOf(logger.Level(0)) {
			return data, nil
		}

		// Return the parsed value
		var level logger.Level
		level.Parse(data.(string))
		return level, nil
	}
}
...
func readConfig[E any](configFilePath string, defaults *E) (*E, error) {
	...
	var config E
	err := vp.Unmarshal(&config, viper.DecodeHook(
		mapstructure.ComposeDecodeHookFunc(
			StringToLoggerLevelHookFunc(),
...
func LoadConfig[E any](defaults *E) (*E, error) {
	...
	if configFileEnv != "" {
		...
		logger.Infof("loading config file from %s", configFileEnv)
		conf, err = readConfig[E](configFileEnv, defaults)

		if err != nil {
			logger.Warnf("failed to load config file specified in ENV '%s' variable. will try to load from standard paths: %s", configFileEnvVar, err)
			loadStandardPaths = true
		}
	} else {
		logger.Infof("ENV '%s' variable not set, will try to load from standard paths", configFileEnvVar)
	}
